...
import Draggable from '../Global/Draggable'
...
const MusicBlockSeeker = ({ musicTitle, musicTrack }) => {
    ...
    useEffect(() => {
        ...
        howlerController.once("end", () => { clearInterval(intervalId) })
        return (() => {
            clearInterval(intervalId)
        })
    }, [])

    const getProgressWidth = (seek_) => ({ width: `${getDateFraction(seek_)}%` })
    const getProgressPosition = (seek_) => ({ left: `${getDateFraction(seek_)}%` })
    ...
    return (
        <div className={styles.music_block_seeker}>
            ...
            <div className={styles.bar}>
                ...
                <Draggable seekTo={seekTo} musicTracK={musicTrack}>
                    <div className={styles.progress_dot} style={{ ...getProgressPosition(seek) }}></div>
                </Draggable>
                <div className={styles.progress_dot_hint} style={{ ...getProgressPosition(seek) }}>{seek}</div>
            </div>
